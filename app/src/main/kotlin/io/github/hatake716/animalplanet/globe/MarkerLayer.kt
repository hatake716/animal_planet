...
/**
 * 表示フィルタ。
 * groups/regions は「チェックした項目だけ表示」。何もチェックしていない(空)ときは絞り込みなし＝全件表示。
 * 「全選択」で全項目を明示的に入れた状態と、「全解除」の空集合は、どちらも全件表示になる。
 * 地域は種ごとに複数持てる(regionMask)ので、チェックした地域のいずれかに生息していれば表示する。
 */
data class MarkerFilter(
    val groups: Set<TaxonGroup> = emptySet(),
    val regions: Set<Int> = emptySet(),
) {
    /** 分類で実際に絞られているか(一部だけ選択)。空と全選択はどちらも「絞っていない」。 */
    fun groupFilterActive(): Boolean = groups.isNotEmpty() && groups.size < TaxonGroup.entries.size
    ...
    private val regionBits: Int = regions.fold(0) { acc, r -> if (r in 0..30) acc or (1 shl r) else acc }

    fun accepts(e: Entry): Boolean {
        if (groups.isNotEmpty() && e.group !in groups) return false
        if (regions.isNotEmpty() && (e.regionMask and regionBits) == 0) return false
        return true
    }
}
...
class MarkerLayer(private val entries: List<Entry>, private val density: Float) {
    ...
    /** 知名度降順→id 順に並べた添字。表示順・代表選出はこの順で走査すれば済む。 */
    ...
    fun update(camera: Camera) {
        ...
        if (!selX.isNaN()) {
            ...
            if (labels.none { it.id == sel }) labels.add(0, Label(e.id, selX, selY, e.name, 9))
        }
        markerCount = k / 8
